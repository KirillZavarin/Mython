...
namespace parse {
    ...
    public:
        ...
        const T& Expect() const {
            ...
            if (token_flow_[current_token].Is<T>()) {
                return token_flow_[current_token].As<T>();
            }
        ...
        void Expect(const U& value) const {
            using namespace std::literals;
            if (token_flow_[current_token].Is<T>() && token_flow_[current_token].As<T>().value == value) {
                return;
            }
            throw LexerError("Not implemented"s);
        }
        ...
        const T& ExpectNext() {
            using namespace std::literals;
            current_token = current_token < token_flow_.size() - 1 ? current_token + 1 : token_flow_.size() - 1;
            return Expect<T>();
        }
    ...
    private:
        ...
        uint16_t indentation_level_ = 0;

        bool IgnoreLine(std::string_view& line) const; //Возваращает true если строка должна игнорироваться

        void AddTokensFromString(std::string_view& line);

        void AddIndent(std::string_view& line);

        void AddNumber(std::string_view& line);

        void AddSymvol(std::string_view& line);

        void AddComparisonOperator(std::string_view& line);
        
        void AddString(std::string_view& line);

        void AddId(std::string_view& line);

        int ReadNumber(std::istringstream& in);
